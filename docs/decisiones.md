...
## 2026-09-23 — Prueba intermitente del salto de pared: los teletransportes de prueba no refrescaban is_on_floor()

**El problema**: `_test_wall_slide_and_wall_jump()` fallaba de forma intermitente (más o menos 1 de cada 3-5 ejecuciones) en la comprobación "saltar desde la pared empuja hacia fuera y hacia arriba". Ya fallaba en el commit anterior a esta rama, así que no lo causó ningún cambio de combate reciente.

**Diagnóstico**: un `CharacterBody2D` solo actualiza `is_on_floor()`/`is_on_wall()` cuando se llama a `move_and_slide()`; escribir `global_position` directamente (como hacen las pruebas para colocar al jugador) no lo toca. Al cargar un nivel de pruebas, el jugador nace de pie sobre el suelo inicial; si un frame de físicas se cuela entre los dos `await process_frame` de `_fresh_level()` (algo que depende de la velocidad de fotogramas y por eso es intermitente), ese primer `move_and_slide()` deja `is_on_floor() == true` de verdad, con el jugador todavía en su punto de aparición. La prueba entonces teletransporta al jugador junto al pozo (`Vector2(2332, -250)`, lejos de cualquier suelo real) sin que nada vuelva a llamar a `move_and_slide()` antes del siguiente frame de físicas. `PlatformerMotor.step()` lee `is_on_floor()` ANTES de llamar a `move_and_slide()` (por diseño: primero se decide el movimiento del frame, luego se aplica), así que ese primer frame tras el teletransporte todavía ve el `true` heredado, pone `velocity.y = 0.0` y recarga el salto extra y el coyote time. Con la velocidad vertical clavada en 0, el salto integrado de Godot para "no perder el contacto con el suelo al bajar una rampa" (el margen `floor_snap_length`) nunca llega a demostrarse a sí mismo que ya no hay suelo debajo: cada frame, con `velocity.y` en 0, Godot lo intenta de nuevo y el estado nunca se corrige solo. Se comprobó con un script de depuración que imprimía `is_on_floor()`, la velocidad y los colisionadores de cada `move_and_slide()`: el estado se quedaba en `true` de forma indefinida, no solo un frame.

Otras pruebas que también teletransportan al jugador (con `global_position =`) no se veían afectadas porque, o bien colocan al jugador sobre suelo real (el "falso suelo" heredado acaba coincidiendo con uno real en cuanto la gravedad lo asienta, unos pocos frames después) o saltan inmediatamente (una velocidad vertical fuertemente negativa hace que Godot deje de intentar el enganche de suelo). El salto de pared es el único caso que se queda quieto, cayendo lentamente junto a una pared vacía, el tiempo suficiente para que el error se note.

**Arreglo**: se añadió un ayudante `_teleport(body, position)` en `tests/smoke_test.gd` que, tras fijar la posición y poner la velocidad a cero, llama una vez a `move_and_slide()` para que `is_on_floor()`/`is_on_wall()` reflejen la posición real de inmediato, y se sustituyeron los 27 teletransportes directos de la prueba (jugador y enemigo) por este ayudante. No se tocó `core/components/platformer_motor.gd`: el problema es exclusivo de teletransportar un `CharacterBody2D` fuera de su propio ciclo de físicas, algo que solo hacen las pruebas (el juego real nunca fija `global_position` directamente).

**Alternativas descartadas**: cambiar el orden de `motor.step()`/`move_and_slide()` en `player.gd` — descartado, porque el orden actual es correcto para el juego real (leer el suelo de "como estaba al empezar este frame" es la forma normal de un controlador cinemático) y el problema solo existe en la prueba. Forzar `is_on_floor()` a un valor concreto — no es posible, Godot no expone un setter público.
