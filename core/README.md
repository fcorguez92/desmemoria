...
| `SheetAnimator` | Anima un `Sprite2D` con hoja de sprites: cada animación es una fila y cada fotograma una columna. Solo presentación | `play(animation)`, `current`; exporta `sprite` y `animations` (nombre → `[fila, nº de fotogramas, fps, bucle]`) |
